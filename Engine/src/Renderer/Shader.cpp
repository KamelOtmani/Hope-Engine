...
static GLenum ShaderTypeFromString(const std::string& type)
{
    ...
    HASSERT(false, "Unknown shader type!");
    return 0;
}
...
std::string Shader::readFile(const std::string& filepath)
{
    ...
    if (in) {
        ...
        HEngineERROR("Could not open file '{0}'", filepath);
    }
...
std::unordered_map<uint32_t, std::string> Shader::preProcess(const std::string& source)
{
    ...
    while (pos != std::string::npos) {
        ...
        HASSERT(eol != std::string::npos, "Syntax error");
        size_t begin = pos + typeTokenLength + 1;
        std::string type = source.substr(begin, eol - begin);
        HASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
...
void Shader::compile(const std::unordered_map<uint32_t, std::string>& shaderSources)
{
    ...
    for (auto& kv : shaderSources) {
        ...
        if (isCompiled == GL_FALSE) {
            ...
            HEngineERROR("{0}", infoLog.data());
            HASSERT(false, "Shader compilation failure!");
            break;
        }
    ...
    if (isLinked == GL_FALSE) {
        ...
        HEngineERROR("{0}", infoLog.data());
        HASSERT(false, "Shader link failure!");
        return;
    }
...
void Shader::UploadUniformFloat2(const std::string& name, const Vec2& value)
{
    GLint location = glGetUniformLocation(m_RendererID, name.c_str());
    glUniform2f(location, value.x, value.y);
}

void Shader::UploadUniformFloat3(const std::string& name, const Vec3& value)
{
    GLint location = glGetUniformLocation(m_RendererID, name.c_str());
    glUniform3f(location, value.x, value.y, value.z);
}

void Shader::UploadUniformFloat4(const std::string& name, const Vec4& value)
{
    GLint location = glGetUniformLocation(m_RendererID, name.c_str());
